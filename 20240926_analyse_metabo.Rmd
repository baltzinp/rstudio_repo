...
# J'analyse les données métaboliques en fonction de la progression ou de la mort
...
metabo_survival %>% 
 filter(site == "os") %>% 
  filter(progression == "non")
```



###Je réalise la même analyse selon si le patient est mort ou non. 


```{r}
label_metabo %>% as.vector()
...
map(as_vector(label_metabo %>% filter(!is.na(label_brut)) %>% select(label_brut)), function(var = .x){
  ...
  geom_jitter(width = .15, heigth = .15, aes( color = dead))+
    labs(title = str_c("Distribution de log(", var, ") en fonction du décès"), y = str_c("log(", var, ")"))+
      scale_y_continuous(trans = "log")+
      facet_grid(~site)+
  theme_philippe()+
    stat_compare_means()-> graph_log
  ...
  geom_boxplot(aes(fill = dead), alpha = .25, outlier.shape = NA)+
  geom_jitter(width = .15, heigth = .15, aes( color = dead))+
    labs(title = str_c("Distribution de ", var, " en fonction du décès"))+
      facet_grid(~site)+
  theme_philippe()+
    stat_compare_means()
...
map(c("tep_0", "tep_2", "tep_4"), function(var = .x){
  ...
  stat_compare_means()+
    ...
    scale_color_brewer(palette = "Set1", direction = -1)+
    scale_y_continuous(trans = "log")+
  labs(title = str_c("Distribution du log de la somme des MTV1 par patient par site pour la ", var), y = "Log de la Somme des MTV des cibles")+
  theme_philippe()
...
metabo_long_sum %>% 
  left_join(code, by = "id") %>% 
  left_join(survive, by = "code") %>% 
  filter(str_detect(parametre, "mtv")) #%>% 
  ggplot(aes(tep, moyenne_cibles))+
  geom_boxplot(aes(fill = tep), alpha = .25)+
  geom_jitter(aes(color = tep), width = .2, height = .2)+
  facet_grid(~site)+
  stat_compare_means()+
  labs()+
  theme_philippe()
